...
AdaptiveRLS<double> compensator(REF_WEIGHT);
TempManager tempSensor(DS_PIN);
InputHandler inputHanler(BUTT_PIN, CALIB_SWITCH_PIN, compensator, external_request, currentState);
        ...
        case SystemState::MEASURE: {                       // измерения температуры и веса
            ...
            if (millis() - sendState_timer >= (is_retry_mode ? DATA_RETRY_PERIOD : ((compensator.isCalibratingMode()) ? DATA_SEND_PERIOD_CALIB : DATA_SEND_PERIOD)) || external_request.force_send) {               // в данном цикле пришло время/нужно принудительно отправлять данные
                ...
                if (restart_reason != 99) {
                    ...
                    // Отправляем трассировку шагов только при аварийных падениях.
                    // Исключаем: 1 (Штатное включение), 3 (Наш ESP.restart()), 0 (Неизвестно)
                    if (restart_reason != 1 && restart_reason != 3 && restart_reason != 0) {
                        msg += ", MainStep";
                        msg += rtc_main_fsm_state;
                        msg += "/Modem: ";
                        msg += rtc_crash_step;
                    }
                    restart_reason = 99;
