...
public:
    // ──────────────────────────── конструктор ──────────────────────────────
    // refVal1 — эталонное значение val1 (целевая точка без ошибки, например 0.0 для нулевого дрейфа).
    // В прошивке референс задаётся через begin()/restoreLiveState(); конструктор с аргументом
    // удобен для тестов и прямого использования без обращения к хранилищу.
