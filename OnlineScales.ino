...
SystemState postModemState = SystemState::SLEEP_SENSORS;
...
void setup() {
    ...
    ModemConfig.rx_pin = MODEM_RX_PIN;                                  // пин RS от SIM800L
    ...
    if (!LittleFS.begin(true)) {                            // инииализируем LittleFS (ляжет под капот FileData)
    ...
    switch (currentState) {
       ...
       case SystemState::MEASURE: {
            ...
            if (external_request == ModificationRequest::START_CALIBRATION) {                   
                ...
                // --- НОВЫЙ БЛОК: Уведомление о старте ---
                String msg = "Переключатель переведен в режим калибровки.%0AНачат алгоритм подбора оптимальной модели компенсации.";
                modemPayload = "user_id=" + String(VK_USER_ID) + "&random_id=" + String(esp_random()) + "&v=5.199&access_token=" + String(VK_TOKEN) + "&message=" + msg;
                
                postModemState = SystemState::CALIBRATION; // После отправки в ВК модем вернет нас в цикл калибровки
                ...
                postModemState = SystemState::SLEEP_SENSORS;        // Стандартный отчет всегда возвращает нас ко сну
                sendState_timer = millis();                             // ОБНОВЛЯЕМ ТАЙМЕР ТОЛЬКО ЗДЕСЬ
                changeFSMState(SystemState::START_MODEM);
        ...
        case SystemState::SLEEP_MODEM:  {
            ...
            if (hasModemError) {
                ...
                changeFSMState(postModemState);              // <-- ПЕРЕХОДИМ ПО УКАЗАННОМУ МАРШРУТУ
                postModemState = SystemState::SLEEP_SENSORS; // Сбрасываем маршрут на дефолтный
            }
            break;
        }
        // ------------------- Особая часть цикла работы, вызывается по таймеру -------------------
        ...
        case SystemState::ERROR_HANDLING: {
            ...
            if (postModemState == SystemState::CALIBRATION) {
        ...
        case SystemState::CALIBRATION:
            ...
            if (external_request == ModificationRequest::END_CALIBRATION) {                                                   
                external_request = ModificationRequest::NONE;
                calibrator.finishCalibration();
                LOG("Calibration finished!");
                
                // --- НОВЫЙ БЛОК: Формируем красивый отчет о результатах калибровки ---
                String msg = "КАЛИБРОВКА ЗАВЕРШЕНА!%0A%0A";
                if (calibData.calibrated) {
                    msg += "Выбрана модель: ";
                    if (calibData.modelType == 0) msg += "Линейная (2 параметра)%0A";
                    else if (calibData.modelType == 1) msg += "Квадратичная (3 параметра)%0A";
                    else msg += "Кубическая (4 параметра)%0A";
                    
                    msg += "%0AКоэффициенты (Theta):%0A";
                    uint8_t n_params = calibData.modelType + 2;
                    for (uint8_t i = 0; i < n_params; i++) {
                        msg += "[" + String(i) + "] = " + String(calibData.params[i], 6) + "%0A";
                    }
                    msg += "%0AСистема обновила энергонезависимую память и теперь использует эту модель.";
                } else {
                    msg += "Отмена: собрано недостаточно данных для построения математической модели.";
                }

                modemPayload = "user_id=" + String(VK_USER_ID) + "&random_id=" + String(esp_random()) + "&v=5.199&access_token=" + String(VK_TOKEN) + "&message=" + msg;
                
                // Возвращаемся в MEASURE (он уже сам решит, уснуть или сразу отправить плановый отчет)
                postModemState = SystemState::MEASURE; 
                changeFSMState(SystemState::START_MODEM);
            }
