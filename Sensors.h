...
class ScalesManager {
  ...
  public:
    ...
    ScalesState sensorTare() {
      if (millis() - _tare_timer < 2000) return ScalesState::ERROR;        // защита от чрезмерного зажатия кнопки - тарировать можно не ранее, чем через 2 секунды
    ...
    void tick() {                                  // по таймауту обновляем фильтр
      float k;
